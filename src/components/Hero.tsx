...
export default function Hero() {
  return (
    <section className="py-12 sm:py-16 md:py-24 -mx-4 sm:-mx-6 px-4 sm:px-6">
      <div className="flex flex-col-reverse lg:grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
        {/* Left column — text */}
        <div>
          <Badge className="mb-6 bg-[#e6f9f0] text-[#0ea86a] border-[#0ea86a]/30 hover:bg-[#d1f5e6] rounded-full px-5 py-2 text-sm font-semibold shadow-sm">
            <span className="w-2.5 h-2.5 bg-[#0ea86a] rounded-full inline-block mr-2 animate-pulse-dot" />
            Disponible para proyectos · Lima, Perú
          </Badge>

          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-[3.25rem] font-extrabold leading-[1.1] mb-6 text-[#0a0f1e]">
            Conecto tu{" "}
            <span className="text-[#1847c2]">planta industrial</span>{" "}
            con soluciones web que generan resultados
          </h1>

          <p className="text-base sm:text-lg text-[#5a6270] mb-8 max-w-xl leading-relaxed">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <Button asChild size="lg" className="bg-[#1847c2] hover:bg-[#1339a0] shadow-lg shadow-[#1847c2]/25 rounded-xl text-base px-8 h-12 min-h-[44px]">
              <Link href="/contacto">
                <span className="w-2 h-2 bg-white rounded-full animate-pulse-dot mr-2" />
                Solicitar demo
              </Link>
            </Button>
            <Button variant="outline" size="lg" asChild className="rounded-xl text-base px-8 h-12 min-h-[44px] border-[#e2e6ee] hover:bg-[#f4f6f9]">
        ...
        <div className="flex justify-center lg:justify-end min-h-[200px]">
          <OEEDashboardMockup />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mt-12 sm:mt-16">
        {kpis.map((kpi) => (
          <Card key={kpi.label} className="border-[#e2e6ee] bg-white rounded-2xl shadow-sm hover:shadow-md transition-shadow">
            <CardContent className="pt-5 pb-4 sm:pt-6 flex flex-col items-center gap-1.5 sm:gap-2">
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-[#f0f4ff] flex items-center justify-center">
                <kpi.icon className={`w-4 h-4 sm:w-5 sm:h-5 ${kpi.color}`} />
              </div>
              <p className={`text-xl sm:text-2xl md:text-3xl font-bold ${kpi.color}`}>{kpi.value}</p>
              <p className="text-xs sm:text-sm text-[#5a6270] text-center">{kpi.label}</p>
            </CardContent>
          </Card>
        ))}
      </div>
